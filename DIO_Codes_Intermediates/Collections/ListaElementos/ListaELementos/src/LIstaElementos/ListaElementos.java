...
//    Codigo Limpo no LISTAELEMENTOS_CODLIMPO
    ...
    public static void main(String[] args) {
        List<String> numerosAleatorios =
                Arrays.asList("3","0","1","3","2","4","7","9","8","6","5");
...
/*        List<Integer> convertNum = numerosAleatorios.stream().map(Integer::parseInt).collect(Collectors.toList()); // TRANSFORMAR DE STRING EM INT - REFERENCE METHOD
        ...
        System.out.println(numerosAleatorios + "\nRetirando os numeros repetidos da lista"); //Primeiro Converter para Int
        List<String>numerosInt = numerosAleatorios.stream()
                .distinct() // a partir desse comando, nao apresenta Strings Repetidos
                .collect(Collectors.toList());
        numerosInt.forEach(System.out::println);

        System.out.println("Mostrar Menor Valor");
        List<Integer> convertNum1 = numerosAleatorios.stream().map(Integer::parseInt).collect(Collectors.toList());
        System.out.println(numerosAleatorios.size());

        System.out.println("Mostrar Maior e Menor Valor");
        List<Integer> convertNum2 = numerosAleatorios.stream().map(Integer::parseInt).collect(Collectors.toList());
        Integer min = convertNum2.get(0);
        Integer max = convertNum2.get(0);
        Integer n = convertNum2.size();

        System.out.println(numerosAleatorios);

        for (int i = 0; i < n; i++) {
            if (convertNum2.get(i) < min) {
                min = convertNum2.get(i);
            }
        }
        System.out.println("Menor valor: " + min);

        for (int i = 0; i < n; i++) {
            if (convertNum2.get(i) > max) {
                max = convertNum2.get(i);
            }
        }
        System.out.println("Maior valor: " + max);

        System.out.println("\n Somar Números Ímpares");
        List<Integer>impar = numerosAleatorios.stream()
                .map(Integer::parseInt)
                .filter(somarImpar -> (somarImpar %2 != 0))
//                .reduce((n1, n2 )-> n1 + n2)
                .collect(Collectors.toList());
